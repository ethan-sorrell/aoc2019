...
(define (bitmap-board board)
  (let* ([right (apply max (map car (hash-keys board)))]
         ...
         [width (- right left)]
         [height (- top bottom)]
         [target (make-bitmap (* 2 (add1 width)) (* 2 (add1 height)))]
         [dc (new bitmap-dc% [bitmap target])]
         [f (new frame% [label "Solution"])])
    (send dc set-scale 2 2)
    (for* ([y (in-range top (+ -1 bottom) -1)]
           [x (in-range right (+ -1 left) -1)])
      (when (not (= 0 (hash-ref board (cons x y) 0)))
        (send dc draw-point x (- y))))
    (send f show #t)
    (void (new message% [parent f] [label (send dc get-bitmap)]))))
...
(define (start-hull-painting)
  (let* ([program (file->string "input.txt")]
    ...
    (let ([state (painting-robot
      ...
      (bitmap-board (run-hull-painting state)))))
