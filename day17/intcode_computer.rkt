...
(provide run-instr change-input file->mem)

(define relative-base 0)

(define (file->mem file-name)
  (let* ([file-string (file->string file-name)]
         [mem-string (substring file-string 0 (sub1 (string-length file-string)))]
         [mem-list (string-split mem-string ",")]
         [mem-list (map string->number mem-list)]
         [mem-pairs (map cons (range (length mem-list)) mem-list)])
    (make-immutable-hash mem-pairs)))
